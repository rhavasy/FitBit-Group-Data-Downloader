...
def MakeApiCall(access_token):
    ...
    file=open(Name+'_results.xml', 'w') #Write results to file
    file.write(response)
    file.close()

if os.path.isfile(Name+'_access_token.txt'): #First check if file exists
    file = open(Name+'_access_token.txt', 'r') #If yes, read token from file
    ...
    file = open(Name+'_access_token.txt', 'w') #write token to file
    file.write(access_token)
    file.close()
    MakeApiCall(access_token)

from xml.etree import ElementTree
with open(Name+'_results.xml', 'rt') as f:
